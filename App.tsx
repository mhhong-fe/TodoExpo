/**
 * React Native 基础知识综合示例 - Todo App
 *
 * 涵盖知识点：
 * - View, Text, TextInput, FlatList
 * - StyleSheet, Flexbox 布局
      ...
      {priorities.map((p) => (
        <TouchableOpacity
          ...
          style={[
            ...
            value === p && { backgroundColor: PRIORITY_COLOR[p] },
          ]}
          onPress={() => onChange(p)}
          activeOpacity={0.7}>
...
const TodoCard: React.FC<TodoCardProps> = ({ item, onToggle, onDelete }) => {
          ...
          <View
            style={[
              ...
              item.done && {
                ...
                borderColor: PRIORITY_COLOR[item.priority],
              },
            ]}>
            {item.done && <Text style={styles.checkmark}>✓</Text>}
          </View>
          <View style={styles.cardTextWrap}>
            <Text style={[styles.cardText, item.done && styles.cardTextDone]} numberOfLines={2}>
              {item.text}
            </Text>
            <Text style={styles.cardMeta}>
              优先级：{PRIORITY_LABEL[item.priority]} · {new Date(item.createdAt).toLocaleTimeString()}
            </Text>
          </View>
        </TouchableOpacity>
        <Pressable
...
const StatsBar: React.FC<StatsBarProps> = ({ total, done }) => {
...
export default function App() {
  ...
  useEffect(() => {
    console.log(`Todo 数量从 ${prevCount} 变为 ${todos.length}`);
  }, [todos.length, prevCount]);
  ...
  const filteredTodos = useMemo(() => {
    return todos
      ...
      .filter((t) =>
        searchText ? t.text.toLowerCase().includes(searchText.toLowerCase()) : true
      )
      .sort((a, b) => {
